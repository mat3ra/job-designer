...
`github:mat3ra/job-designer#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/job-designer/releases/download/wip-<short-sha>/job-designer.tgz
```
...
```bash
npm run mat3ra:install -- job-designer wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/job-designer@<release-url>` with
