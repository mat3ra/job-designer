...
export function setReducerDeps(deps) {
    if (deps.AccountsSelector)
        reducerDeps.accountsSelector = deps.AccountsSelector;
    if (deps.createOrUpdate)
        reducerDeps.createOrUpdate = deps.createOrUpdate;
    ...
    if (deps.Router)
        reducerDeps.router = deps.Router;
    if (deps.setIsLoading)
        reducerDeps.setIsLoading = deps.setIsLoading;
    if (deps.submitJob)
        reducerDeps.submitJobAPI = deps.submitJob;
    if (deps.terminateJob)
        reducerDeps.terminateJobAPI = deps.terminateJob;
}
