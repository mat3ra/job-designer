...
import { MATERIAL_SWITCH, MATERIALS_SET, MATERIALS_ADD, MATERIALS_REMOVE, MATERIALS_UPDATE_INDEX, } from "../actions/Material";
...
function materialsSet(state, action, materialsToRetain = [], resetWorkflowContexts = true) {
    ...
    if (resetWorkflowContexts) {
        state.workflowContexts = materials.map(() => deepClone(state.workflowContexts[state.index] || {}));
        state.index = 0;
    }
    ...
    renderJobForDesignerState({ ...state, materials, materialsSet: action.materialsSet, index: state.index }, job, (_a = action.metaProperties) !== null && _a !== void 0 ? _a : []);
    return {
        ...state,
        materials,
...
function materialsAdd(state, action) {
    ...
    action.materials.forEach(() => state.workflowContexts.push(deepClone(state.workflowContexts[state.index] || {})));
...
function materialsRemove(state, action) {
    ...
    return materialsSet(state, action, [], { index: state.index >= materials.length ? state.index - 1 : state.index }, false);
}
/**
