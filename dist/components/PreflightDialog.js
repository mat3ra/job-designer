...
export default function PreflightDialog({ open, onClose, getContext, onSubmit, onGoToStep, }) {
    ...
    return (_jsxs(Dialog, { open: open, onClose: onClose, maxWidth: "sm", fullWidth: true, id: "job-preflight-dialog", children: [_jsxs(DialogTitle, { sx: { pb: 1 }, children: [_jsx(Typography, { variant: "h6", component: "span", children: "Preflight" }), _jsx(Typography, { variant: "body2", color: "text.secondary", id: "job-preflight-summary", sx: { display: "block" }, children: isRunning ? "Running checks…" : getReportSummary(report, acknowledged) })] }), isRunning ? _jsx(LinearProgress, {}) : null, _jsx(DialogContent, { dividers: true, children: _jsxs(Stack, { divider: _jsx(Box, { sx: { borderTop: "1px solid", borderColor: "divider" } }), children: [((_a = report === null || report === void 0 ? void 0 : report.rows) !== null && _a !== void 0 ? _a : []).map((row) => (_jsx(PreflightRowView, { row: row, isAcknowledged: acknowledged.includes(row.id), isExpanded: expandedRowId === row.id, onToggleExpanded: () => setExpandedRowId(expandedRowId === row.id ? null : row.id), onAcknowledge: () => setAcknowledged([...acknowledged, row.id]), onFix: handleFix }, row.id))), !report && !isRunning ? (_jsx(Typography, { variant: "body2", color: "text.secondary", sx: { py: 2 }, children: "No checks ran." })) : null] }) }), _jsxs(DialogActions, { children: [_jsx(Button, { onClick: onClose, children: "Back to designer" }), _jsx(Button, { onClick: run, disabled: isRunning, id: "job-preflight-rerun", children: "Re-run checks" }), _jsx(Button, { variant: "contained", onClick: onSubmit, disabled: !canSubmit || isRunning, id: "job-preflight-submit", children: "Submit job" })] })] }));
}
