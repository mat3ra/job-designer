...
export function trackEvent(event: AnalyticsEvent, properties?: AnalyticsProperties): void {
    const { trackEvent: track } = getInjectedDeps() as { trackEvent?: TrackEventFunction };
 ...
 */
export function durationSince(startedAtMs?: number, nowMs?: number): number | undefined {
    if (!startedAtMs) return undefined;

    const elapsed = ((nowMs ?? Date.now()) - startedAtMs) / 1000;
...
export function summarizeReportForAnalytics(report: {
    ...
    const countOf = (state: string) => report.rows.filter((row) => row.state === state).length;
